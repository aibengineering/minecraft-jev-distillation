...
face, how to move, how fast, whether to jump, what to do with its hands),
carries out the answer and records everything. Two policies exist:

- **Live jev.** [jev](https://typesafe.ai) (TypeSafe) reads the fight as prose
  with the clocks that matter (sword cooldown, shield activation, a creeper's
  fuse, an archer's draw, arrows in flight and where they land) and answers
  over the API in about 300 ms.
- **The distilled model.** Five LightGBM heads trained on jev's answers read
  the same fight as a row of 414 numbers and answer in 0.2 ms. Every jev
  window is stored as a training row, so the answers jev is paid for are
  bought once.

[Mine Labs](https://github.com/aibengineering/mine-labs) builds the arenas,
summons the enemies and judges the outcome. A few perception routines are
ported from [mine-ai-mcp](https://github.com/aibengineering/mine-ai-mcp).
